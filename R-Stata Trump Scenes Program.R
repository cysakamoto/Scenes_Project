# Transforming stata to another format
...
#Installed
library(readstata13)
# mydata <- readstata13::read.dta13(file = "ScenesWithVotingFixed.dta", convert.factors = TRUE,
...
mydata <- merge(mydataE, Merge34[, c("zipcode", setdiff(colnames(Merge34), colnames(mydataE)))], by="zipcode")

# mydata <- merge(mydata, Merge34, by="zipcode")

hyesun <- read.csv("nhgis0030_ts_geog2010_zcta.csv", header = TRUE, sep = ",", skipNul = TRUE, blank.lines.skip = TRUE, dec = ".")

names(hyesun)[3] <- paste('zipcode')

mydata <- merge(mydata, hyesun[, c("zipcode", setdiff(colnames(hyesun), colnames(mydata)))], by="zipcode")

# mydata <- merge(mydata, hyesun, by="zipcode")

library(foreign)

finalvars <- read.spss("FinalVariables_plusYP_clean1.sav",
                      to.data.frame = TRUE)

mydata <- merge(mydata, finalvars[, c("zipcode", setdiff(colnames(finalvars), colnames(mydata)))], by="zipcode")

bizzip <- read.spss("bizzip_part_arts11.sav", to.data.frame = TRUE)

mydata <- merge(mydata, bizzip[, c("zipcode", setdiff(colnames(bizzip), colnames(mydata)))], by="zipcode")

# mydata <- merge(mydata, bizzip, by="zipcode")

dan_new <- read.csv("DanFile_NEW.csv", header = TRUE, sep = ",",
                   skipNul = TRUE, blank.lines.skip = TRUE, dec = ".")

# mydata <- merge(mydata, finalvars, by="zipcode")

write.csv(mydata, file="trump_data", na = "", row.names=FALSE, append = TRUE, sep = ",")

write.foreign(mydata, "mydata.txt", "mydata.sps",   package="SPSS")

#NOW WE SHOULD HAVE ALL THE DATA NEEDED FOR OUR TESTS 
#Let's set up our data for our regressions. 
...
mydata <- merge(mydata, trumpdata1[, c("zipcode", setdiff(colnames(trumpdata1), colnames(mydata)))], by="zipcode")

newtrump <- merge(dan_new, trumpdata1, by = "countcen")

Merge34_1 <- Merge34[,c("zipcode", "pctfemale2k", "MedAge2k", "pctchild2k", 
                        "NonWhite_2000", "LevelNonWhite_2000", "ZYP_FactorScore", "pctba2k")]

crimemerge <- Merge34[,c("zipcode", "CrimeRate1999county")]

selection <- mydata[, c("zipcode", "estimateginiindex", "countcen", "donaldjtrump_prop16", "donaldjtrump_2016",
                    "percent_nonwhite2k", "estimateginiindex", "pctfemale2k", "MedAge2k", "pctchild2k", 
                    "NonWhite_2000", "LevelNonWhite_2000", "ZYP_FactorScore", "pctba2k", "CrimeRate1999county", 
                    "TotalRepubVotes2008")]
...
selection <- hansel_final[, c("zipcode", "estimateginiindex", "countcen", "donaldjtrump_prop16", "donaldjtrump_2016",
                        "percent_nonwhite2k", "estimateginiindex", "pctfemale2k", "MedAge2k", "pctchild2k", 
                        "NonWhite_2000", "LevelNonWhite_2000", "ZYP_FactorScore", "pctba2k", "CrimeRate1999county", 
                        "TotalRepubVotes2008")]

write.csv(selection, file="trump_hansel_variables", na = "", row.names=FALSE, append = TRUE, sep = ",")
...
zpctnw2k <- scale(pctnw2k)
zreligneg <- scale(religneg)

testh1 <- data.frame(zdensity, zpctnw2k, zreligneg,  row.names = NULL)
